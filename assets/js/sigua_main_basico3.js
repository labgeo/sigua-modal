...
/* Listado de edificios para Sidebar edificios*/
$(document).ready(function () {
	$.getJSON(servidor+'/api/pub/edificio/all/items', function(data) {
			// get json, parse it and append to .nav
			j=data.length;		  		  
			//alert('total: '+j);		  
			$.each(data, function (i, item) {
				var numEdi = item.id.substring(1, 2);// De 0001, 0037 sale 0, De 0101 y 1101 sale 1			
				// par o impar
				if (numEdi%2==0) {
					$("#ediList").append('<li class="list-group-item" style=" padding: 1px 5px;"><a href="#" onclick="selEdi(\''+item.bbox+'\')" ><small><mark>' + item.id+'</mark> '+item.nombre+'</small></a></li>'); 
				} else {
					$("#ediList").append('<li class="list-group-item list-group-item-info" style=" padding: 1px 5px;"><a href="#" onclick="selEdi(\''+item.bbox+'\')" ><small><mark>' + item.id+'</mark> '+item.nombre+'</small></a></li>'); 
				}
			})
	})     
});
