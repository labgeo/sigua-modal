...
$( "#edificios" ).focus(function() {
	...
	$.getJSON('http://www.sigua.ua.es/api/pub/edificio/all/items', function(data) {
		  j=data.length;		  		  
		  //alert('total: '+j);		  
		  //$("#edificios").append('<option>(Selecciona un edificio)</option>');		   
		  $.each(data, function(i,item) {
			  $("#edificios").append('<option value="'+item.bbox+'">' + item.nombre + '</option>');	 
		  })
...
$(document).ready(function () {
	$.getJSON('http://www.sigua.ua.es/api/pub/edificio/all/items', function(data) {
		  ...
		  $("#edificios").append('<option>(Selecciona un edificio)</option>');		   
		  $.each(data, function(i,item) {
			  //$("#edificios").append('<option onclick="selEdi(\''+item.bbox+'\')" > '+ item.id +' '+ item.nombre +'</option>'); 
			  //$("#edificios").append('<option onclick="selEdi(\''+item.bbox+'\')" > '+ item.id +' '+ item.nombre +'</option>');
			  $("#edificios").append('<option value="'+item.bbox+'">' + item.nombre + '</option>');
